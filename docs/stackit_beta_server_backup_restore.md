...
```
  ...
  $ stackit beta server backup restore xxx --server-id=zzz --start-server-after-restore=true
```
...
```
  ...
  -i, --volume-ids string            Backup volume ids, as comma separated UUID values.
```
