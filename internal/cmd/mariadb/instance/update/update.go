		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				...
				if !errors.As(err, &dsaInvalidPlanError) {
					return fmt.Errorf("build MariaDB instance update request: %w", err)
				}
				return err
			}
			err = req.Execute()
			if err != nil {
				return fmt.Errorf("update MariaDB instance: %w", err)
			}
...
func buildRequest(ctx context.Context, model *inputModel, apiClient mariaDBClient) (mariadb.ApiPartialUpdateInstanceRequest, error) {
	...
	req = req.PartialUpdateInstancePayload(mariadb.PartialUpdateInstancePayload{
