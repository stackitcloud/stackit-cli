...
func fixtureRequest(mods ...func(request *sqlserverflex.ApiListDatabasesRequest)) sqlserverflex.ApiListDatabasesRequest {
	request := testClient.DefaultAPI.ListDatabases(testCtx, testProjectId, testRegion, testInstanceId)
	for _, mod := range mods {
		mod(&request)
	}
	return request
...
func TestOutputResult(t *testing.T) {
	type args struct {
		...
		instanceId   string
		projectLabel string
		databases    []sqlserverflex.ListDatabase
	}
	tests := []struct {
		name    string
		args    args
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := outputResult(params.Printer, tt.args.outputFormat, tt.args.instanceId, tt.args.projectLabel, tt.args.databases); (err != nil) != tt.wantErr {
