...
func NewCmd(params *types.CmdParams) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "list",
		Short: "Lists supported SQLServer Flex user roles of an instance",
		Long:  "Lists supported SQLServer Flex user roles of an instance.",
		Args:  args.NoArgs,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("get SQLServer Flex user roles: %w", err)
			}
			roles := resp.Roles
			...
			return outputResult(params.Printer, model.OutputFormat, instanceLabel, roles)
		},
	}
...
func buildRequest(ctx context.Context, model *inputModel, apiClient *sqlserverflex.APIClient) sqlserverflex.ApiListRolesRequest {
	return apiClient.DefaultAPI.ListRoles(ctx, model.ProjectId, model.Region, *model.InstanceId)
}

func outputResult(p *print.Printer, outputFormat, instanceLabel string, roles []string) error {
	return p.OutputResult(outputFormat, roles, func() error {
		if len(roles) == 0 {
			p.Outputf("No user roles found for instance %q\n", instanceLabel)
			return nil
		}
