...
func NewCmd(params *types.CmdParams) *cobra.Command {
	cmd := &cobra.Command{
		...
		Short: "Deletes the given TelemetryRouter instance",
		Long:  "Deletes the given TelemetryRouter instance.",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			err = req.Execute()
			if err != nil {
				return fmt.Errorf("delete TelemetryRouter instance: %w", err)
			}

			// Wait for async operation, if async mode not enabled
			if !model.Async {
				err := spinner.Run(params.Printer, "Deleting instance", func() error {
					_, err = wait.DeleteTelemetryRouterWaitHandler(ctx, apiClient.DefaultAPI, model.ProjectId, model.Region, model.InstanceID).WaitWithContext(ctx)
					return err
				})
				if err != nil {
					return fmt.Errorf("wait for TelemetryRouter instance deletion: %w", err)
				}
			}
...
func buildRequest(ctx context.Context, model *inputModel, apiClient *telemetryrouter.APIClient) telemetryrouter.ApiDeleteTelemetryRouterRequest {
	req := apiClient.DefaultAPI.DeleteTelemetryRouter(ctx, model.ProjectId, model.Region, model.InstanceID)
	return req
}
