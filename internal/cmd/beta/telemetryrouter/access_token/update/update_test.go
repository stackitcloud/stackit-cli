...
func fixtureRequest(mods ...func(request *telemetryrouter.ApiUpdateAccessTokenRequest)) telemetryrouter.ApiUpdateAccessTokenRequest {
...
func fixturePayload(mods ...func(payload *telemetryrouter.UpdateAccessTokenPayload)) telemetryrouter.UpdateAccessTokenPayload {
	payload := telemetryrouter.UpdateAccessTokenPayload{
		DisplayName: *telemetryrouter.NewNullableString(new(testDisplayName)),
		Description: *telemetryrouter.NewNullableString(new(testDescription)),
	}
	for _, mod := range mods {
		mod(&payload)
	}
...
func TestParseInput(t *testing.T) {
	tests := []struct {
		...
		{
			description: "required only (no values to update)",
			argValues:   fixtureArgValues(),
...
func TestBuildRequest(t *testing.T) {
	tests := []struct {
		description     string
		model           *inputModel
		expectedRequest telemetryrouter.ApiUpdateAccessTokenRequest
	}{
		{
			description:     "base",
		...
		{
			description: "display name only leaves description unset (not explicit null)",
			model: fixtureInputModel(func(model *inputModel) {
				model.Description = nil
			}),
			expectedRequest: fixtureRequest(func(request *telemetryrouter.ApiUpdateAccessTokenRequest) {
				*request = request.UpdateAccessTokenPayload(fixturePayload(func(payload *telemetryrouter.UpdateAccessTokenPayload) {
					payload.Description = telemetryrouter.NullableString{}
				}))
			}),
		},
		{
			description: "description only leaves display name unset (not explicit null)",
			model: fixtureInputModel(func(model *inputModel) {
				model.DisplayName = nil
			}),
			expectedRequest: fixtureRequest(func(request *telemetryrouter.ApiUpdateAccessTokenRequest) {
				*request = request.UpdateAccessTokenPayload(fixturePayload(func(payload *telemetryrouter.UpdateAccessTokenPayload) {
					payload.DisplayName = telemetryrouter.NullableString{}
				}))
			}),
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.description, func(t *testing.T) {
			...
			diff := cmp.Diff(request, tt.expectedRequest,
				cmp.AllowUnexported(tt.expectedRequest, telemetryrouter.NullableString{}),
