	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if model.Create != nil && *model.Create {
				if !model.AssumeYes {
					prompt := fmt.Sprintf("Are you sure you want to create and attach a network interface to server %q?", serverLabel)
				...
				p.Info("Created and attached a network interface to server %q\n", serverLabel)
				return nil
			}
