	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if model.Delete != nil && *model.Delete {
				if !model.AssumeYes {
					prompt := fmt.Sprintf("Are you sure you want to detach and delete all network interfaces from server %q? (This cannot be undone)", serverLabel)
				...
				p.Info("Detached and deleted all network interfaces from server %q\n", serverLabel)
				return nil
			}
