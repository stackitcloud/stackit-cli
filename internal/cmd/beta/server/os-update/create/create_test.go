...
var testMaintenanceWindow = 13 // some value between 1-24

func fixtureFlagValues(mods ...func(flagValues map[string]string)) map[string]string {
	flagValues := map[string]string{
		projectIdFlag:         testProjectId,
		serverIdFlag:          testServerId,
		maintenanceWindowFlag: strconv.Itoa(testMaintenanceWindow),
	}
...
func fixtureInputModel(mods ...func(model *inputModel)) *inputModel {
	model := &inputModel{
		...
		MaintenanceWindow: int64(testMaintenanceWindow),
	}
	for _, mod := range mods {
		mod(model)
	}
...
func fixturePayload(mods ...func(payload *serverupdate.CreateUpdatePayload)) serverupdate.CreateUpdatePayload {
	payload := serverupdate.CreateUpdatePayload{
		MaintenanceWindow: utils.Ptr(int64(testMaintenanceWindow)),
	}
	for _, mod := range mods {
		mod(&payload)
	}
	...
	tests := []struct {
		...
		},
		{
			...
			expectedModel: fixtureInputModel(func(model *inputModel) {
				model.MaintenanceWindow = defaultMaintenanceWindow
			}),
		},
		{
