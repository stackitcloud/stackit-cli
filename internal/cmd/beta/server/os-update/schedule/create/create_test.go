...
func fixtureFlagValues(mods ...func(flagValues map[string]string)) map[string]string {
	flagValues := map[string]string{
		...
		maintenanceWindowFlag: strconv.Itoa(defaultMaintenanceWindow),
	}
...
func fixtureInputModel(mods ...func(model *inputModel)) *inputModel {
	model := &inputModel{
		...
		Rrule:             defaultRrule,
		MaintenanceWindow: int64(defaultMaintenanceWindow),
	}
	for _, mod := range mods {
		mod(model)
	}
...
func fixturePayload(mods ...func(payload *serverupdate.CreateUpdateSchedulePayload)) serverupdate.CreateUpdateSchedulePayload {
	payload := serverupdate.CreateUpdateSchedulePayload{
		...
		MaintenanceWindow: utils.Ptr(int64(defaultMaintenanceWindow)),
	}
	for _, mod := range mods {
		mod(&payload)
	}
