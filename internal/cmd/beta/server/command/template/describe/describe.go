...
func NewCmd(p *print.Printer) *cobra.Command {
	cmd := &cobra.Command{
		...
		Example: examples.Build(
			examples.NewExample(
				`Get details of a Server Command Template with name "RunShellScript" for server "xxx"`,
				"$ stackit beta server command template describe RunShellScript --server-id=xxx"),
			examples.NewExample(
				`Get details of a Server Command Template with name "RunShellScript" for server "xxx" in json format`,
				"$ stackit beta server command template describe RunShellScript --server-id=xxx -o json"),
		),
