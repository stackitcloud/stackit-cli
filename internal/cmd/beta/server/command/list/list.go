...
func outputResult(p *print.Printer, outputFormat string, commands []runcommand.Commands) error {
	...
	default:
		...
		table.SetHeader("ID", "STATUS", "STARTED_AT", "FINISHED_AT", "TEMPLATE NAME", "TEMPLATE TITLE")
		for i := range commands {
			s := commands[i]
			table.AddRow(*s.Id, *s.Status, *s.StartedAt, *s.FinishedAt, *s.CommandTemplateName, *s.CommandTemplateTitle)
		}
		err := table.Display(p)
		if err != nil {
