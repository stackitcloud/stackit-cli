...
func fixtureRequest(mods ...func(request *automation.ApiDeleteVolumeAutomationRequest)) automation.ApiDeleteVolumeAutomationRequest {
	request := testClient.DefaultAPI.DeleteVolumeAutomation(testCtx, testProjectId, testRegion, testAutomationId)
	for _, mod := range mods {
		mod(&request)
	}
	return request
...
func TestParseInput(t *testing.T) {
	tests := []struct {
		...
		{
			description: "with assume yes",
			argValues:   fixtureArgValues(),
			flagValues: fixtureFlagValues(func(flagValues map[string]string) {
				flagValues[globalflags.AssumeYesFlag] = "true"
			}),
			isValid: true,
...
func TestBuildRequest(t *testing.T) {
	tests := []struct {
		...
		model           *inputModel
		isValid         bool
		expectedRequest automation.ApiDeleteVolumeAutomationRequest
	}{
		{
			description:     "base",
