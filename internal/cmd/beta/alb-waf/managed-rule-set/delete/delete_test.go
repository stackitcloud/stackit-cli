...
func fixtureRequest(mods ...func(request *albwaf.ApiDeleteManagedRuleSetRequest)) albwaf.ApiDeleteManagedRuleSetRequest {
	request := testClient.DefaultAPI.DeleteManagedRuleSet(testCtx, testProjectId, testRegion, testName)
	for _, mod := range mods {
		mod(&request)
	}
	return request
...
func TestBuildRequest(t *testing.T) {
	tests := []struct {
		description    string
		model          *inputModel
		expectedResult albwaf.ApiDeleteManagedRuleSetRequest
	}{
		{
			description:    "base",
