...
func NewCmd(params *types.CmdParams) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			prompt := fmt.Sprintf("Are you sure you want to create a Key Value Store (valkey) instance for project %q?", projectLabel)
			...
			if err != nil {
				if _, ok := errors.AsType[*cliErr.DSAInvalidPlanError](err); !ok {
					return fmt.Errorf("build Key Value Store (valkey) instance creation request: %w", err)
				}
				return err
			}
			resp, err := req.Execute()
			if err != nil {
				return fmt.Errorf("create Key Value Store (valkey) instance: %w", err)
			}
			...
			if !model.Async {
				...
				if err != nil {
					return fmt.Errorf("wait for Key Value Store (valkey) instance creation: %w", err)
				}
			}
