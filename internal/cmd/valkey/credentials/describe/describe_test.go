...
func fixtureArgValues(mods ...func(argValues []string)) []string {
	argValues := []string{testCredentialsId}
...
func fixtureRequest(mods ...func(request *valkey.ApiGetCredentialsRequest)) valkey.ApiGetCredentialsRequest {
...
func TestParseInput(t *testing.T) {
	tests := []struct {
		...
		{
			description: "credentials id invalid",
...
func TestBuildRequest(t *testing.T) {
	tests := []struct {
		description     string
		model           *inputModel
		expectedRequest valkey.ApiGetCredentialsRequest
	}{
		{
			description:     "base",
...
func TestOutputResult(t *testing.T) {
	tests := []struct {
		name        string
		credentials *valkey.CredentialsResponse
		wantErr     bool
	}{
		{
		...
		{
			name: "credentials without raw",
			credentials: &valkey.CredentialsResponse{
				Id:  testCredentialsId,
				Uri: "redis://host:6379",
			},
			wantErr: false,
		},
		{
			name: "credentials with raw",
			credentials: &valkey.CredentialsResponse{
				Id:  testCredentialsId,
				Uri: "redis://host:6379",
				Raw: &valkey.RawCredentials{
					Credentials: valkey.Credentials{
						...
						Username: "user",
						Password: "secret",
						Uri:      new("redis://user:secret@host:6379"),
					},
				},
			},
			wantErr: false,
		...
		{
			name: "credentials with raw but empty username",
			credentials: &valkey.CredentialsResponse{
				Id:  testCredentialsId,
				Uri: "redis://host:6379",
				Raw: &valkey.RawCredentials{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := outputResult(params.Printer, "", tt.credentials); (err != nil) != tt.wantErr {
