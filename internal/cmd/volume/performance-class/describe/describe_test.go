...
func fixtureRequest(mods ...func(request *iaas.ApiGetVolumePerformanceClassRequest)) iaas.ApiGetVolumePerformanceClassRequest {
	request := testClient.DefaultAPI.GetVolumePerformanceClass(testCtx, testProjectId, testRegion, testVolumePerformanceClass)
	for _, mod := range mods {
		mod(&request)
	}
	return request
...
func TestOutputResult(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := outputResult(params.Printer, tt.args.outputFormat, tt.args.performanceClass); (err != nil) != tt.wantErr {
