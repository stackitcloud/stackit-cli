		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !model.Async {
				err := spinner.Run(params.Printer, "Unrescuing server", func() error {
					_, err = wait.UnrescueServerWaitHandler(ctx, apiClient.DefaultAPI, model.ProjectId, model.Region, model.ServerId).WaitWithContext(ctx)
					return err
				})
				if err != nil {
...
func buildRequest(ctx context.Context, model *inputModel, apiClient *iaas.APIClient) iaas.ApiUnrescueServerRequest {
	return apiClient.DefaultAPI.UnrescueServer(ctx, model.ProjectId, model.Region, model.ServerId)
}
