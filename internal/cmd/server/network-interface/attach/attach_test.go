...
func fixtureRequestAttach(mods ...func(request *iaas.ApiAddNicToServerRequest)) iaas.ApiAddNicToServerRequest {
	request := testClient.DefaultAPI.AddNicToServer(testCtx, testProjectId, testRegion, testServerId, testNicId)
	for _, mod := range mods {
		mod(&request)
	}
	return request
...
func fixtureRequestCreateAndAttach(mods ...func(request *iaas.ApiAddNetworkToServerRequest)) iaas.ApiAddNetworkToServerRequest {
	request := testClient.DefaultAPI.AddNetworkToServer(testCtx, testProjectId, testRegion, testServerId, testNetworkId)
	for _, mod := range mods {
		mod(&request)
	}
	return request
