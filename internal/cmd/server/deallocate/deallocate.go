		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !model.Async {
				err := spinner.Run(params.Printer, "Deallocating server", func() error {
					_, err = wait.DeallocateServerWaitHandler(ctx, apiClient.DefaultAPI, model.ProjectId, model.Region, model.ServerId).WaitWithContext(ctx)
					return err
				})
				if err != nil {
...
func buildRequest(ctx context.Context, model *inputModel, apiClient *iaas.APIClient) iaas.ApiDeallocateServerRequest {
	return apiClient.DefaultAPI.DeallocateServer(ctx, model.ProjectId, model.Region, model.ServerId)
}
