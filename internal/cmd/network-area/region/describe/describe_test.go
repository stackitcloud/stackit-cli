...
func fixtureRequest(mods ...func(request *iaas.ApiGetNetworkAreaRegionRequest)) iaas.ApiGetNetworkAreaRegionRequest {
	request := testClient.DefaultAPI.GetNetworkAreaRegion(testCtx, testOrgId, testAreaId, testRegion)
	for _, mod := range mods {
		mod(&request)
	}
	return request
...
func Test_outputResult(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := outputResult(params.Printer, tt.args.outputFormat, tt.args.region, tt.args.areaId, tt.args.networkAreaLabel, tt.args.regionalArea); (err != nil) != tt.wantErr {
