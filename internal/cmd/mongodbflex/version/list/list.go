...
func NewCmd(params *types.CmdParams) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "list",
		Short: "Lists MongoDB Flex versions",
		Long:  "Lists MongoDB Flex versions.",
		Args:  args.NoArgs,
...
func buildRequest(ctx context.Context, model *inputModel, apiClient mongodbflex.DefaultAPI) mongodbflex.ApiListVersionsRequest {
	return apiClient.ListVersions(ctx, model.ProjectId, model.Region)
}

func outputResult(p *print.Printer, outputFormat string, versions []string) error {
	return p.OutputResult(outputFormat, versions, func() error {
		if len(versions) == 0 {
			p.Outputf("No MongoDB Flex versions found.")
			return nil
		}
