...
func buildRequest(ctx context.Context, service string, model *inputModel, apiClient MongoDBFlexClient) (mongodbflex.ApiPartialUpdateInstanceRequest, error) {
	...
	if model.FlavorId == nil && (model.RAM != nil || model.CPU != nil) {
		...
		flavorId, err = mongodbflexUtils.LoadFlavorId(service, *cpu, *ram, flavors.Flavors)
		if err != nil {
	...
	if model.StorageClass != nil || model.StorageSize != nil {
		...
		err = mongodbflexUtils.ValidateStorage(service, model.StorageClass, model.StorageSize, storages, *validationFlavorId)
		if err != nil {
			return req, err
		}
	}
