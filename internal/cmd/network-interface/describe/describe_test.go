...
func fixtureRequest(mods ...func(request *iaas.ApiGetNicRequest)) iaas.ApiGetNicRequest {
	request := testClient.DefaultAPI.GetNic(testCtx, testProjectId, testRegion, testNetworkId, testNicId)
	for _, mod := range mods {
		mod(&request)
	}
	return request
...
func Test_outputResult(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := outputResult(params.Printer, tt.args.outputFormat, tt.args.nic); (err != nil) != tt.wantErr {
