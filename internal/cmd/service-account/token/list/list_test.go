...
func fixtureRequest(mods ...func(request *serviceaccount.ApiListAccessTokensRequest)) serviceaccount.ApiListAccessTokensRequest {
	request := testClient.ListAccessTokens(testCtx, testProjectId, testServiceAccountEmail)
	for _, mod := range mods {
		mod(&request)
	}
	return request
...
func TestOutputResult(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := outputResult(params.Printer, tt.args.outputFormat, tt.args.tokensMetadata); (err != nil) != tt.wantErr {
