...
func TestParseInput(t *testing.T) {
	tests := []struct {
		...
		{
			description: "ttl days invalid",
			flagValues: fixtureFlagValues(func(flagValues map[string]string) {
				flagValues[ttlDaysFlag] = "invalid"
			}),
			isValid: false,
		},
	}
...
func TestBuildRequest(t *testing.T) {
	tests := []struct {
		...
		model           *inputModel
		isValid         bool
		expectedRequest serviceaccount.ApiCreateAccessTokenRequest
	}{
		{
			description:     "base",
...
func TestOutputResult(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := outputResult(params.Printer, tt.args.outputFormat, tt.args.serviceAccountEmail, tt.args.token); (err != nil) != tt.wantErr {
