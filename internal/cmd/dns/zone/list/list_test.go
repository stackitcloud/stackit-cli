...
func fixtureRequest(mods ...func(request *dns.ApiGetZonesRequest)) dns.ApiGetZonesRequest {
	request := testClient.GetZones(testCtx, testProjectId)
...
func TestBuildRequest(t *testing.T) {
	tests := []struct {
		...
		model           *flagModel
		page            int
		expectedRequest dns.ApiGetZonesRequest
	}{
		{
			description:     "base",
		...
		},
		{
			...
			page:            1,
			expectedRequest: testClient.GetZones(testCtx, testProjectId).Page(1).PageSize(100),
		},
	}
