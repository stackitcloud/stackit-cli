...
func fixtureRequest(mods ...func(request *dns.ApiGetRecordSetRequest)) dns.ApiGetRecordSetRequest {
	request := testClient.DefaultAPI.GetRecordSet(testCtx, testProjectId, testZoneId, testRecordSetId)
	for _, mod := range mods {
		mod(&request)
	}
	return request
...
func TestBuildRequest(t *testing.T) {
	tests := []struct {
		...
		model           *inputModel
		isValid         bool
		expectedRequest dns.ApiGetRecordSetRequest
	}{
		{
			description:     "base",
...
func TestOutputResult(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := outputResult(params.Printer, tt.args.outputFormat, tt.args.recordSet); (err != nil) != tt.wantErr {
