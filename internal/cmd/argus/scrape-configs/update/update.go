...
func NewCmd(p *print.Printer) *cobra.Command {
	cmd := &cobra.Command{
		...
		Short: "Updates a Scrape Config of an Argus instance",
		Long: fmt.Sprintf("%s\n%s\n%s",
			"Updates a Scrape Config of an Argus instance.",
		...
		Example: examples.Build(
			examples.NewExample(
				`Update a Scrape Config from Argus instance "xxx", using an API payload sourced from the file "./payload.json"`,
				"$ stackit argus scrape-configs update my-config --payload @./payload.json --instance-id xxx"),
			examples.NewExample(
				`Update an Scrape Config from Argus instance "xxx", using an API payload provided as a JSON string`,
				`$ stackit argus scrape-configs update my-config --payload "{...}" --instance-id xxx`),
			examples.NewExample(
				`Generate a payload with the current values of a Scrape Config, and adapt it with custom values for the different configuration options`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !model.AssumeYes {
				prompt := fmt.Sprintf("Are you sure you want to update scrape config %q?", model.JobName)
			...
			p.Info("%s Argus Scrape Config %q\n", operationState, model.JobName)
			return nil
		},
	}
	configureFlags(cmd)
